...
if "experiment_id" not in st.session_state:
    st.warning("Please select an experiment from the Experiment History page.")
    st.stop()
...
if not analysis:
    st.warning("Could not load dataset analysis for default values. Using basic inputs.")
...
if not feature_cols:
    st.error("No features recorded for this experiment.")
    st.stop()
    ...
    for idx, feature in enumerate(feature_cols):
        ...
        with cols[idx % 3]:
            if col_type == "numeric" or feature in num_summary:
                ...
                if not options:
                    options = ["Unknown"]
                input_data[feature] = st.selectbox(f"{feature} (Categorical)", options, key=f"input_{feature}")
...
if submit:
    with st.spinner("Running prediction..."):
        ...
        if result and "prediction" in result:
            ...
            st.metric("Predicted Value", str(result["prediction"]))
            ...
            if result.get("feature_contributions"):
                st.write("**Model Feature Importances (Global):**")
